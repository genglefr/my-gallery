        ...
        .directive('ngImageGallery', ['$rootScope', '$timeout', '$q', 'ngImageGalleryOpts',
            function($rootScope, $timeout, $q, ngImageGalleryOpts){
                return {
                    ...
                    // Images container
                    '<div class="galleria-images img-anim-{{imgAnim}} img-move-dir-{{_imgMoveDirection}}" ng-repeat="image in images track by image.id" ng-if="_activeImg == image">'+
                    '<img class="galleria-image" id="{{image._id}}" ng-right-click ng-src="{{image.url}}" ondragstart="return false;" ng-attr-alt="{{image.alt || undefined}}"></img>'+
                    '<span class="heart">{{image.likes}}</span></div>'+
                    ...
                    '<div class="ng-image-gallery-errorplaceholder" ng-bind-html="errorPlaceHolder | ngImageGalleryTrust"></div>'+
                    '</div>'+
                    '</div>'+
                    '</div>',
                            ...
                            **/
                            ...
                            scope._setActiveImg = function(imgObj){
                                ...
                                scope._loadImg(imgObj).then(function(imgObj){
                                    ...
                                    //scope._activeImageIndex = scope.images.indexOf(imgObj);
                                    //scope.imgError = true;
                                    // ??? Check how this can happen
                                    scope._activeImg = scope.images[0];
                            ...
                            **/
                            ...
                            if(window.Hammer){
                                ...
                                hammerElem.on('doubletap', function(ev){
                                    ...
                                    $timeout(function(){
                                        if(scope.methods.doubletap){
                                            scope.methods.doubletap(ev.target.id);
                                        } else {
                                            scope.methods.close();
                                        }
                                    });
